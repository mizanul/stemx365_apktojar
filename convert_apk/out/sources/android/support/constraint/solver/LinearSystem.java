...
public class LinearSystem {
    ...
    public SolverVariable createObjectVariable(Object anchor) {
        ...
        if (anchor instanceof ConstraintAnchor) {
            ...
            if (variable.f15id == -1 || variable.f15id > this.mVariablesID || this.mCache.mIndexedVariables[variable.f15id] == null) {
                if (variable.f15id != -1) {
                    variable.reset();
                }
    ...
    private int optimize(Goal goal) {
        ...
        while (!done) {
            ...
            if (pivotCandidate != null) {
                if (this.mAlreadyTestedCandidates[pivotCandidate.f15id]) {
                    pivotCandidate = null;
                } else {
                    this.mAlreadyTestedCandidates[pivotCandidate.f15id] = true;
                    tested++;
    ...
    private int enforceBFS(Goal goal) throws Exception {
        ...
        if (infeasibleSystem) {
            ...
            while (!done) {
                ...
                while (i2 < this.mNumRows) {
                    ArrayRow current = this.mRows[i2];
                    if (current.variable.mType != SolverVariable.Type.UNRESTRICTED && current.constantValue < f) {
                        int j = 1;
        ...
        while (true) {
            ...
            if (this.mRows[i4].variable.mType != SolverVariable.Type.UNRESTRICTED) {
                if (this.mRows[i4].constantValue < 0.0f) {
                    break;
                }
            }
            i4++;
    ...
    private void displayRows() {
        ...
        String s = "";
        for (int i = 0; i < this.mNumRows; i++) {
            s = (s + this.mRows[i]) + Base64.LINE_SEPARATOR;
        }
        if (this.mGoal.variables.size() != 0) {
            s = s + this.mGoal + Base64.LINE_SEPARATOR;
        }
    ...
    public void displayReadableRows() {
        ...
        String s = "";
        for (int i = 0; i < this.mNumRows; i++) {
            s = (s + this.mRows[i].toReadableString()) + Base64.LINE_SEPARATOR;
        }
        if (this.mGoal != null) {
            s = s + this.mGoal + Base64.LINE_SEPARATOR;
        }
    ...
    private void displaySolverVariables() {
        ...
        System.out.println(s + Base64.LINE_SEPARATOR);
    }

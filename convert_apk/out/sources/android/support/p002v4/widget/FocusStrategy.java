...
class FocusStrategy {
    ...
    private static boolean isToDirectionOf(int direction, Rect src2, Rect dest) {
        if (direction != 17) {
            if (direction != 33) {
                if (direction != 66) {
                    if (direction != 130) {
                        throw new IllegalArgumentException("direction must be one of {FOCUS_UP, FOCUS_DOWN, FOCUS_LEFT, FOCUS_RIGHT}.");
                    } else if (src2.bottom <= dest.top) {
                        return true;
                    } else {
                        return false;
                    }
                } else if (src2.right <= dest.left) {
                    return true;
                } else {
                    return false;
                }
            } else if (src2.top >= dest.bottom) {
                return true;
            } else {
                return false;
            }
        } else if (src2.left >= dest.right) {
            return true;
        } else {
            return false;
        }

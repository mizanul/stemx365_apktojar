...
public abstract class TypeToken<T> extends TypeCapture<T> implements Serializable {
    ...
    protected TypeToken(Class<?> declaringClass) {
        ...
        if (captured instanceof Class) {
            ...
            this.runtimeType = m133of(declaringClass).resolveType(captured).runtimeType;
        }
    }
    ...
    /* renamed from: of */
    public static <T> TypeToken<T> m133of(Class<T> type) {
        return new SimpleTypeToken(type);
    }

    /* renamed from: of */
    public static TypeToken<?> m134of(Type type) {
        return new SimpleTypeToken(type);
    }
    ...
    public final <X> TypeToken<T> where(TypeParameter<X> typeParam, TypeToken<X> typeArg) {
        return new SimpleTypeToken(new TypeResolver().where(ImmutableMap.m62of(typeParam.typeVariable, typeArg.runtimeType)).resolve(this.runtimeType));
    }

    public final <X> TypeToken<T> where(TypeParameter<X> typeParam, Class<X> typeArg) {
        return where(typeParam, m133of(typeArg));
    }
    ...
    public final TypeToken<?> resolveType(Type type) {
        ...
        return m134of(resolver.resolve(type));
    }
    ...
    public final TypeToken<? extends T> getSubtype(Class<?> subclass) {
        ...
        return m134of(resolveTypeArgsForSubclass(subclass));
    }
    ...
    public Object writeReplace() {
        return m134of(new TypeResolver().resolve(this.runtimeType));
    }
    ...
    private static boolean isAssignableToParameterizedType(Type from, ParameterizedType to) {
        ...
        TypeToken<?> fromTypeToken = m134of(from);
    ...
    static <T> TypeToken<? extends T> toGenericType(Class<T> cls) {
        if (cls.isArray()) {
            return m134of(Types.newArrayType(toGenericType(cls.getComponentType()).runtimeType));
        }
        TypeVariable<Class<T>>[] typeParams = cls.getTypeParameters();
        if (typeParams.length > 0) {
            return m134of((Type) Types.newParameterizedType(cls, typeParams));
        }
        return m133of(cls);
    }
    ...
    private TypeToken<? super T> getSupertypeFromUpperBounds(Class<? super T> supertype, Type[] upperBounds) {
        for (Type upperBound : upperBounds) {
            TypeToken<?> of = m134of(upperBound);
            if (m133of(supertype).isAssignableFrom(of)) {
    ...
    private TypeToken<? extends T> getSubtypeFromLowerBounds(Class<?> subclass, Type[] lowerBounds) {
        ...
        if (0 < arr$.length) {
            return m134of(arr$[0]).getSubtype(subclass);
        }
        throw new IllegalArgumentException(subclass + " isn't a subclass of " + this);
    }

    private TypeToken<? super T> getArraySupertype(Class<? super T> supertype) {
        return m134of(newArrayClassOrGenericArrayType(((TypeToken) Preconditions.checkNotNull(getComponentType(), "%s isn't a super type of %s", supertype, this)).getSupertype(supertype.getComponentType()).runtimeType));
    }

    private TypeToken<? extends T> getArraySubtype(Class<?> subclass) {
        return m134of(newArrayClassOrGenericArrayType(getComponentType().getSubtype(subclass.getComponentType()).runtimeType));
    }

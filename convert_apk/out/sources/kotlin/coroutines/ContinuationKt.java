...
public final class ContinuationKt {
    ...
    private static final <T> void resume(Continuation<? super T> $this$resume, T value) {
        ...
        $this$resume.resumeWith(Result.m188constructorimpl(value));
    }

    private static final <T> void resumeWithException(Continuation<? super T> $this$resumeWithException, Throwable exception) {
        Result.Companion companion = Result.Companion;
        $this$resumeWithException.resumeWith(Result.m188constructorimpl(ResultKt.createFailure(exception)));
    }
    ...
    public static final <T> void startCoroutine(Function1<? super Continuation<? super T>, ? extends Object> $this$startCoroutine, Continuation<? super T> completion) {
        ...
        Result.Companion companion = Result.Companion;
        intercepted.resumeWith(Result.m188constructorimpl(unit));
    }
    ...
    private static final <T> Object suspendCoroutine(Function1<? super Continuation<? super T>, Unit> block, Continuation<? super T> c) {
        InlineMarker.mark(0);
        SafeContinuation safe = new SafeContinuation(IntrinsicsKt.intercepted(c));
        block.invoke(safe);
        Object orThrow = safe.getOrThrow();
        if (orThrow == IntrinsicsKt.getCOROUTINE_SUSPENDED()) {
            DebugProbesKt.probeCoroutineSuspended(c);
        }
        InlineMarker.mark(1);
        return orThrow;
    }

...
public class CommandLineLoader {
    ...
    private void parseArgv() {
        ...
        for (String argument : list.subList(1, list.size())) {
    ...
    private void parseRemappingArguments() {
        for (String remapping : this.remappingArguments) {
            ...
            if (remap.length > 2) {
                ...
                this.remappings.put(GraphName.m181of(remap[0]), GraphName.m181of(remap[1]));
            }
        }
    }
    ...
    private NameResolver buildParentResolver() {
        ...
        if (this.specialRemappings.containsKey(CommandLineVariables.ROS_NAMESPACE)) {
            namespace = GraphName.m181of(this.specialRemappings.get(CommandLineVariables.ROS_NAMESPACE)).toGlobal();
        } else if (this.environment.containsKey(EnvironmentVariables.ROS_NAMESPACE)) {
            namespace = GraphName.m181of(this.environment.get(EnvironmentVariables.ROS_NAMESPACE)).toGlobal();
        }

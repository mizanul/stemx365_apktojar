...
public class DefaultPublisher<T> extends DefaultTopicParticipant implements Publisher<T> {
    ...
    public DefaultPublisher(NodeIdentifier nodeIdentifier2, TopicDeclaration topicDeclaration, MessageSerializer<T> serializer, MessageFactory messageFactory2, ScheduledExecutorService executorService) {
        ...
        ListenerGroup<PublisherListener<T>> listenerGroup = new ListenerGroup<>(executorService);
    ...
    public ChannelBuffer finishHandshake(ConnectionHeader incomingHeader) {
        ...
        Preconditions.checkState(incomingType.equals(expectedType) || incomingType.equals(Subscriber.TOPIC_MESSAGE_TYPE_WILDCARD), "Unexpected message type " + incomingType + " != " + expectedType);
        ...
        if (incomingChecksum.equals(expectedChecksum) || incomingChecksum.equals(Subscriber.TOPIC_MESSAGE_TYPE_WILDCARD)) {

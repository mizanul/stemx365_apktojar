...
public class ParameterServer {
    ...
    private final GraphName masterName = GraphName.m181of("/master");
    ...
    private Set<GraphName> getSubtreeNames(GraphName parent, Map<String, Object> subtree, Set<GraphName> names) {
        for (String name : subtree.keySet()) {
            ...
            if (possibleSubtree instanceof Map) {
                names.addAll(getSubtreeNames(parent.join(GraphName.m181of(name)), (Map) possibleSubtree, names));
            } else {
                names.add(parent.join(GraphName.m181of(name)));
            }
        }
        return names;
    }

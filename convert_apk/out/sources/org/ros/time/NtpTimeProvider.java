...
public class NtpTimeProvider implements TimeProvider {
    ...
    public NtpTimeProvider(InetAddress host2, ScheduledExecutorService scheduledExecutorService2) {
        ...
        this.ntpClient = nTPUDPClient;
        nTPUDPClient.setDefaultTimeout(Videoio.CAP_QT);
        this.offset = 0;
    ...
    public void updateTime() throws IOException {
        ...
        long longValue = ((Long) CollectionMath.median(offsets)).longValue();
        this.offset = longValue;
        log.info(String.format("NTP time offset: %d ms", new Object[]{Long.valueOf(longValue)}));
    }
    ...
    private long computeOffset() throws IOException {
        try {
            ...
            Log log2 = log;
            log2.error("Failed to read time from NTP server: " + this.host.getHostName(), e);
            throw e;
        }
    }

...
public class ContourFitting extends Algorithm {
    ...
    public void estimateTransformation(Mat src2, Mat dst, Mat alphaPhiST, double[] dist, boolean fdContour) {
        double[] dist_out = new double[1];
        estimateTransformation_0(this.nativeObj, src2.nativeObj, dst.nativeObj, alphaPhiST.nativeObj, dist_out, fdContour);
        if (dist != null) {
            dist[0] = dist_out[0];
        }
    }

    public void estimateTransformation(Mat src2, Mat dst, Mat alphaPhiST, double[] dist) {
        double[] dist_out = new double[1];
        estimateTransformation_1(this.nativeObj, src2.nativeObj, dst.nativeObj, alphaPhiST.nativeObj, dist_out);
        if (dist != null) {
            dist[0] = dist_out[0];
        }
    }

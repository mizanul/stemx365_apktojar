...
public class SparsePyrLKOpticalFlow extends SparseOpticalFlow {
    ...
    public static SparsePyrLKOpticalFlow create(Size winSize, int maxLevel, TermCriteria crit, int flags, double minEigThreshold) {
        Size size = winSize;
        TermCriteria termCriteria = crit;
        return __fromPtr__(create_0(size.width, size.height, maxLevel, termCriteria.type, termCriteria.maxCount, termCriteria.epsilon, flags, minEigThreshold));
    }

    public static SparsePyrLKOpticalFlow create(Size winSize, int maxLevel, TermCriteria crit, int flags) {
        return __fromPtr__(create_1(winSize.width, winSize.height, maxLevel, crit.type, crit.maxCount, crit.epsilon, flags));
    }

    public static SparsePyrLKOpticalFlow create(Size winSize, int maxLevel, TermCriteria crit) {
        return __fromPtr__(create_2(winSize.width, winSize.height, maxLevel, crit.type, crit.maxCount, crit.epsilon));
    }

    public static SparsePyrLKOpticalFlow create(Size winSize, int maxLevel) {
        return __fromPtr__(create_3(winSize.width, winSize.height, maxLevel));
    }

    public static SparsePyrLKOpticalFlow create(Size winSize) {
        return __fromPtr__(create_4(winSize.width, winSize.height));
    }

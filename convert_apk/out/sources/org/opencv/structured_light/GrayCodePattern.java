...
public class GrayCodePattern extends StructuredLightPattern {
    ...
    public static GrayCodePattern create(int width, int height) {
        return __fromPtr__(create_0(width, height));
    }
    ...
    public boolean getProjPixel(List<Mat> patternImages, int x, int y, Point projPix) {
        ...
        boolean retVal = getProjPixel_0(this.nativeObj, Converters.vector_Mat_to_Mat(patternImages).nativeObj, x, y, projPix_out);
        if (projPix != null) {
            projPix.f175x = projPix_out[0];
            projPix.f176y = projPix_out[1];
        }
        return retVal;
    }

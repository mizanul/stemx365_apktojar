...
public class Subdiv2D {
    ...
    public Subdiv2D(Rect rect) {
        this.nativeObj = Subdiv2D_1(rect.f180x, rect.f181y, rect.width, rect.height);
    }

    public void initDelaunay(Rect rect) {
        initDelaunay_0(this.nativeObj, rect.f180x, rect.f181y, rect.width, rect.height);
    }

    public int insert(Point pt) {
        return insert_0(this.nativeObj, pt.f175x, pt.f176y);
    }
    ...
    public int locate(Point pt, int[] edge, int[] vertex) {
        ...
        int retVal = locate_0(this.nativeObj, pt.f175x, pt.f176y, edge_out, vertex_out);
        if (edge != null) {
    ...
    public int findNearest(Point pt, Point nearestPt) {
        ...
        int retVal = findNearest_0(this.nativeObj, pt.f175x, pt.f176y, nearestPt_out);
        if (nearestPt != null) {
            nearestPt.f175x = nearestPt_out[0];
            nearestPt.f176y = nearestPt_out[1];
        }
        return retVal;
    }

    public int findNearest(Point pt) {
        return findNearest_1(this.nativeObj, pt.f175x, pt.f176y);
    }
    ...
    public int edgeOrg(int edge, Point orgpt) {
        ...
        if (orgpt != null) {
            orgpt.f175x = orgpt_out[0];
            orgpt.f176y = orgpt_out[1];
        }
        return retVal;
    }
    ...
    public int edgeDst(int edge, Point dstpt) {
        ...
        if (dstpt != null) {
            dstpt.f175x = dstpt_out[0];
            dstpt.f176y = dstpt_out[1];
        }
        return retVal;
    }

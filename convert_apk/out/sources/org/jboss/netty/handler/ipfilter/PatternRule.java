...
public class PatternRule implements IpFilterRule, Comparable<Object> {
    ...
    public boolean contains(InetAddress inetAddress) {
        ...
        Pattern pattern2 = this.ipPattern;
        if (pattern2 != null && pattern2.matcher(inetAddress.getHostAddress()).matches()) {
            return true;
        }
        Pattern pattern3 = this.namePattern;
        if (pattern3 == null || !pattern3.matcher(inetAddress.getHostName()).matches()) {
            return false;
        }
        return true;
    }
    ...
    private void parse(String pattern2) {
        if (pattern2 != null) {
            String ip = "";
            String name = "";
            for (String c : pattern2.split(",")) {
                String c2 = c.trim();
                if (c2.equals("n:localhost")) {
                    this.localhost = true;
    ...
    private static boolean isLocalhost(InetAddress address) {
        ...
        try {
            for (InetAddress addr : InetAddress.getAllByName(Address.LOOPBACK)) {
    ...
    public int compareTo(Object o) {
        ...
        if (str != null) {
            return str.compareTo(p.getPattern());
        }
        return -1;
    }
}

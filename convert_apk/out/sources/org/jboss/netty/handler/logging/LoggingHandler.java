...
public class LoggingHandler implements ChannelUpstreamHandler, ChannelDownstreamHandler {
    ...
    static {
        ...
        while (true) {
            ...
            int padding2 = strArr2.length - i3;
    ...
    private static String formatBuffer(ChannelBuffer buf) {
        ChannelBuffer channelBuffer = buf;
        int length = buf.readableBytes();
        StringBuilder dump = new StringBuilder(((length / 16) + (length % 15 == 0 ? 0 : 1) + 4) * 80);
        ...
        while (i < endIndex) {
            ...
            dump.append(BYTE2HEX[channelBuffer.getUnsignedByte(i)]);
            if (relIdxMod16 == 15) {
                dump.append(" |");
                for (int j = i - 15; j <= i; j++) {
                    dump.append(BYTE2CHAR[channelBuffer.getUnsignedByte(j)]);
                }
                dump.append('|');
            }
            i++;
        }
        if ((15 & (i - startIndex)) != 0) {
            int remainder = length & 15;
            dump.append(HEXPADDING[remainder]);
            dump.append(" |");
            for (int j2 = i - remainder; j2 < i; j2++) {
                dump.append(BYTE2CHAR[channelBuffer.getUnsignedByte(j2)]);
            }

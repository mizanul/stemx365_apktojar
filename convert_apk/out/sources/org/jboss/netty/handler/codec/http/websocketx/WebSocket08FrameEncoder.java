...
public class WebSocket08FrameEncoder extends OneToOneEncoder {
    ...
    public Object encode(ChannelHandlerContext ctx, Channel channel, Object msg) throws Exception {
        ...
        if (frame instanceof TextWebSocketFrame) {
            opcode = 1;
        } else if ((frame instanceof PingWebSocketFrame) != 0) {
            opcode = 9;
        } else if ((frame instanceof PongWebSocketFrame) != 0) {
            opcode = 10;
        } else if ((frame instanceof CloseWebSocketFrame) != 0) {
            opcode = 8;
        } else if ((frame instanceof BinaryWebSocketFrame) != 0) {
            opcode = 2;
        } else if ((frame instanceof ContinuationWebSocketFrame) != 0) {
            opcode = 0;
        } else {

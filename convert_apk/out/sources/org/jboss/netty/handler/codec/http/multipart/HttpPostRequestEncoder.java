...
public class HttpPostRequestEncoder implements ChunkedInput {
    ...
    public void addBodyHttpData(InterfaceHttpData data) throws ErrorDataEncoderException {
        ...
        String str2;
        boolean localMixed;
        InterfaceHttpData interfaceHttpData = data;
            ...
            if (!this.isMultipart) {
                ...
                if (this.duringMixedMode) {
                    ...
                    if (fileUpload3 == null || !fileUpload3.getName().equals(fileUpload2.getName())) {
                        ...
                        str = "Content-Disposition: file; filename=\"";
                    } else {
                        localMixed = true;
                        str2 = "Content-Disposition: form-data; name=\"";
                        str = "Content-Disposition: file; filename=\"";
                    ...
                    if (fileUpload4 == null || !fileUpload4.getName().equals(fileUpload2.getName())) {
                        ...
                        InternalAttribute pastAttribute = (InternalAttribute) list.get(list.size() - 2);
                        str2 = "Content-Disposition: form-data; name=\"";
                        this.globalBodySize -= (long) pastAttribute.size();
                        StringBuilder sb = new StringBuilder();
                        sb.append(((str2 + encodeAttribute(fileUpload2.getName(), this.charset) + "\"\r\n") + "Content-Type: multipart/mixed; boundary=" + this.multipartMixedBoundary + "\r\n\r\n") + "--" + this.multipartMixedBoundary + "\r\n");
                        str = "Content-Disposition: file; filename=\"";
                        sb.append(str);
                        sb.append(encodeAttribute(fileUpload2.getFilename(), this.charset));
                        sb.append("\"\r\n");
                ...
                if (localMixed) {
                    internal3.addValue("--" + this.multipartMixedBoundary + "\r\n");
                    internal3.addValue(str + encodeAttribute(fileUpload2.getFilename(), this.charset) + "\"\r\n");
                } else {
                    internal3.addValue("--" + this.multipartDataBoundary + "\r\n");
                    internal3.addValue(str2 + encodeAttribute(fileUpload2.getName(), this.charset) + "\"; " + HttpPostBodyUtil.FILENAME + "=\"" + encodeAttribute(fileUpload2.getFilename(), this.charset) + "\"\r\n");
                }
        ...
        if (!this.headerFinalized) {
            ...
            if (contentTypes != null) {
                ...
                for (String contentType : contentTypes) {
                    if (!contentType.toLowerCase().startsWith(HttpHeaders.Values.MULTIPART_FORM_DATA) && !contentType.toLowerCase().startsWith(HttpHeaders.Values.APPLICATION_X_WWW_FORM_URLENCODED)) {
            ...
            if (this.isMultipart) {
                ...
                this.request.addHeader("Content-Type", HttpHeaders.Values.APPLICATION_X_WWW_FORM_URLENCODED);
            }
    ...
    private HttpChunk encodeNextChunkMultipart(int sizeleft) throws ErrorDataEncoderException {
        ...
        if (interfaceHttpData instanceof InternalAttribute) {
            try {
                buffer = ChannelBuffers.wrappedBuffer(((InternalAttribute) interfaceHttpData).toString().getBytes("ASCII"));
            ...
            if (interfaceHttpData instanceof Attribute) {
                try {
                    buffer = ((Attribute) interfaceHttpData).getChunk(sizeleft);
                } catch (IOException e2) {
                    throw new ErrorDataEncoderException((Throwable) e2);
                }
            } else {
                try {
                    buffer = ((FileUpload) interfaceHttpData).getChunk(sizeleft);
        ...
        if (channelBuffer == null) {
            ...
            this.currentBuffer = ChannelBuffers.wrappedBuffer(channelBuffer, buffer);
        }
        if (this.currentBuffer.readableBytes() >= HttpPostBodyUtil.chunkSize) {
    ...
    private HttpChunk encodeNextChunkUrlEncoded(int sizeleft) throws ErrorDataEncoderException {
        ...
        int size = sizeleft;
        if (this.isKey) {
            buffer = ChannelBuffers.wrappedBuffer(interfaceHttpData.getName().getBytes());
            this.isKey = false;
            ChannelBuffer channelBuffer = this.currentBuffer;
            if (channelBuffer == null) {
                ...
                this.currentBuffer = ChannelBuffers.wrappedBuffer(channelBuffer, buffer, ChannelBuffers.wrappedBuffer("=".getBytes()));
                size -= buffer.readableBytes() + 1;
            }
            if (this.currentBuffer.readableBytes() >= HttpPostBodyUtil.chunkSize) {
                return new DefaultHttpChunk(fillChannelBuffer());
        ...
        try {
            ...
            if (buffer2.capacity() == 0) {
                ...
                if (channelBuffer2 == null) {
                    ...
                    this.currentBuffer = ChannelBuffers.wrappedBuffer(channelBuffer2, delimiter);
                }
                if (this.currentBuffer.readableBytes() >= HttpPostBodyUtil.chunkSize) {
                    return new DefaultHttpChunk(fillChannelBuffer());
                }
            ...
            if (channelBuffer3 == null) {
                ...
                this.currentBuffer = ChannelBuffers.wrappedBuffer(channelBuffer3, buffer2, delimiter);
            } else {
                this.currentBuffer = ChannelBuffers.wrappedBuffer(channelBuffer3, buffer2);
            }
            if (this.currentBuffer.readableBytes() >= HttpPostBodyUtil.chunkSize) {
                return new DefaultHttpChunk(fillChannelBuffer());
            }
            ...
            ChannelBuffer channelBuffer4 = buffer;
            throw new ErrorDataEncoderException((Throwable) e);
        }
    }
    ...
    public HttpChunk nextChunk() throws ErrorDataEncoderException {
        ...
        ChannelBuffer channelBuffer = this.currentBuffer;
        if (channelBuffer != null) {
            size -= channelBuffer.readableBytes();
        }
        if (size <= 0) {
            return new DefaultHttpChunk(fillChannelBuffer());
        }

...
public abstract class FrameDecoder extends SimpleChannelUpstreamHandler implements LifeCycleAwareChannelHandler {
    ...
    public ChannelBuffer appendToCumulation(ChannelBuffer input) {
        ...
        ChannelBuffer wrappedBuffer = ChannelBuffers.wrappedBuffer(cumulation2, input);
        ChannelBuffer input2 = wrappedBuffer;
    ...
    public ChannelBuffer updateCumulation(ChannelHandlerContext ctx2, ChannelBuffer input) {
        ...
        if (readableBytes > 0) {
            ...
            if (readableBytes < inputCapacity && inputCapacity > this.copyThreshold) {
                ChannelBuffer newCumulationBuffer = newCumulationBuffer(ctx2, input.readableBytes());
                ChannelBuffer newCumulation = newCumulationBuffer;
                this.cumulation = newCumulationBuffer;
                newCumulationBuffer.writeBytes(input);
                return newCumulation;
            } else if (input.readerIndex() != 0) {
                ChannelBuffer slice = input.slice();
                ChannelBuffer newCumulation2 = slice;
        ...
        try {
            ...
            if (cumulation2 != null) {
                ...
                if (cumulation2.readable()) {
                    callDecode(ctx2, ctx2.getChannel(), cumulation2, (SocketAddress) null);
                }
                Object partialFrame = decodeLast(ctx2, ctx2.getChannel(), cumulation2);
                if (partialFrame != null) {
                    unfoldAndFireMessageReceived(ctx2, (SocketAddress) null, partialFrame);
                }
                ctx2.sendUpstream(e);
            }
        } finally {

...
public final class ChannelBuffers {
    ...
    static {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            cArr[(i << 1) + 0] = DIGITS[(i >>> 4) & 15];
            cArr[(i << 1) + 1] = DIGITS[(i >>> 0) & 15];
        }
    }
    ...
    public static ChannelBuffer copiedBuffer(ByteOrder endianness, CharSequence string, Charset charset) {
        if (string == null) {
            throw new NullPointerException(StringSerializer.STRING_TAG);
    ...
    public static int compare(ChannelBuffer bufferA, ChannelBuffer bufferB) {
        ChannelBuffer channelBuffer = bufferA;
        ChannelBuffer channelBuffer2 = bufferB;
        int aLen = bufferA.readableBytes();
        ...
        if (bufferA.order() == bufferB.order()) {
            for (int i = uintCount; i > 0; i--) {
                long va = channelBuffer.getUnsignedInt(aIndex);
                long vb = channelBuffer2.getUnsignedInt(bIndex);
                if (va > vb) {
                    return 1;
                }
                if (va < vb) {
            ...
            for (int i2 = uintCount; i2 > 0; i2--) {
                long va2 = channelBuffer.getUnsignedInt(aIndex);
                long vb2 = ((long) swapInt(channelBuffer2.getInt(bIndex))) & 4294967295L;
                if (va2 > vb2) {
                    return 1;
                }
                if (va2 < vb2) {
        ...
        for (int i3 = byteCount; i3 > 0; i3--) {
            short va3 = channelBuffer.getUnsignedByte(aIndex);
            short vb3 = channelBuffer2.getUnsignedByte(bIndex);
            if (va3 > vb3) {
                return 1;
            }
            if (va3 < vb3) {
    ...
    public static int swapInt(int value) {
        return (swapShort((short) value) << 16) | (swapShort((short) (value >>> 16)) & UShort.MAX_VALUE);
    }
    ...
    static ByteBuffer encodeString(CharBuffer src2, Charset charset) {
        CharsetEncoder encoder = CharsetUtil.getEncoder(charset);
        ByteBuffer dst = ByteBuffer.allocate((int) (((double) src2.remaining()) * ((double) encoder.maxBytesPerChar())));
        try {
            CoderResult cr = encoder.encode(src2, dst, true);
            if (!cr.isUnderflow()) {
                cr.throwException();
            }
    ...
    static String decodeString(ByteBuffer src2, Charset charset) {
        CharsetDecoder decoder = CharsetUtil.getDecoder(charset);
        CharBuffer dst = CharBuffer.allocate((int) (((double) src2.remaining()) * ((double) decoder.maxCharsPerByte())));
        try {
            CoderResult cr = decoder.decode(src2, dst, true);
            if (!cr.isUnderflow()) {
                cr.throwException();
            }

...
public class ByteBufferBackedChannelBuffer extends AbstractChannelBuffer {
    ...
    public int getUnsignedMedium(int index) {
        return ((getByte(index) & 255) << 16) | ((getByte(index + 1) & 255) << 8) | ((getByte(index + 2) & 255) << 0);
    }
    ...
    public void setBytes(int index, ChannelBuffer src2, int srcIndex, int length) {
        if (src2 instanceof ByteBufferBackedChannelBuffer) {
            ByteBuffer data = ((ByteBufferBackedChannelBuffer) src2).buffer.duplicate();
            data.limit(srcIndex + length).position(srcIndex);
            setBytes(index, data);
        } else if (this.buffer.hasArray()) {
            src2.getBytes(srcIndex, this.buffer.array(), this.buffer.arrayOffset() + index, length);
        } else {
            src2.getBytes(srcIndex, (ChannelBuffer) this, index, length);
        }
    }
    ...
    public void setBytes(int index, byte[] src2, int srcIndex, int length) {
        ByteBuffer data = this.buffer.duplicate();
        data.limit(index + length).position(index);
        data.put(src2, srcIndex, length);
    }

    public void setBytes(int index, ByteBuffer src2) {
        ByteBuffer data = this.buffer.duplicate();
        data.limit(src2.remaining() + index).position(index);
        data.put(src2);
    }
    ...
    public ChannelBuffer copy(int index, int length) {
        try {
            ByteBuffer src2 = (ByteBuffer) this.buffer.duplicate().position(index).limit(index + length);

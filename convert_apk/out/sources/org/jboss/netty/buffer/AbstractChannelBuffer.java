...
public abstract class AbstractChannelBuffer implements ChannelBuffer {
    ...
    public void setBytes(int index, byte[] src2) {
        setBytes(index, src2, 0, src2.length);
    }

    public void setBytes(int index, ChannelBuffer src2) {
        setBytes(index, src2, src2.readableBytes());
    }

    public void setBytes(int index, ChannelBuffer src2, int length) {
        if (length <= src2.readableBytes()) {
            setBytes(index, src2, src2.readerIndex(), length);
            src2.readerIndex(src2.readerIndex() + length);
            return;
        }
        throw new IndexOutOfBoundsException("Too many bytes to write: Need " + length + ", maximum is " + src2.readableBytes());
    }
    ...
    public void writeBytes(byte[] src2, int srcIndex, int length) {
        setBytes(this.writerIndex, src2, srcIndex, length);
        this.writerIndex += length;
    }

    public void writeBytes(byte[] src2) {
        writeBytes(src2, 0, src2.length);
    }

    public void writeBytes(ChannelBuffer src2) {
        writeBytes(src2, src2.readableBytes());
    }

    public void writeBytes(ChannelBuffer src2, int length) {
        if (length <= src2.readableBytes()) {
            writeBytes(src2, src2.readerIndex(), length);
            src2.readerIndex(src2.readerIndex() + length);
            return;
        }
        throw new IndexOutOfBoundsException("Too many bytes to write - Need " + length + ", maximum is " + src2.readableBytes());
    ...
    public void writeBytes(ChannelBuffer src2, int srcIndex, int length) {
        setBytes(this.writerIndex, src2, srcIndex, length);
        this.writerIndex += length;
    }

    public void writeBytes(ByteBuffer src2) {
        int length = src2.remaining();
        setBytes(this.writerIndex, src2);
        this.writerIndex += length;
    }

...
public class NioDatagramChannelFactory implements DatagramChannelFactory {
    ...
    public DatagramChannel newChannel(ChannelPipeline pipeline) {
        ...
        return new NioDatagramChannel(this, pipeline, nioDatagramPipelineSink, nioDatagramPipelineSink.nextWorker(), this.family);
    }

    public void releaseExternalResources() {
        WorkerPool<NioDatagramWorker> workerPool2 = this.workerPool;
        if (workerPool2 instanceof ExternalResourceReleasable) {
            ((ExternalResourceReleasable) workerPool2).releaseExternalResources();
        }
    }
}

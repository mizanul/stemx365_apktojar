...
public class HttpTunnelingServlet extends HttpServlet {
    ...
    public void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        if (!"POST".equalsIgnoreCase(req.getMethod())) {
            if (logger.isWarnEnabled()) {
                ...
                internalLogger.warn("Unallowed method: " + req.getMethod());
            }
            res.sendError(Videoio.CAP_PROP_XI_TRG_SOFTWARE);
            return;
        }
        ...
        if (!future.isSuccess()) {
            if (logger.isWarnEnabled()) {
                ...
                internalLogger2.warn("Endpoint unavailable: " + cause.getMessage(), cause);
            }
            res.sendError(503);
            return;
        }
        ...
        try {
            res.setStatus(200);
            res.setHeader("Content-Type", HttpPostBodyUtil.DEFAULT_BINARY_CONTENT_TYPE);
    ...
    private static ChannelBuffer read(PushbackInputStream in) throws IOException {
        ...
        } else if (bytesToRead != 0 || (b = in.read()) < 0 || in.available() < 0) {
            return null;
        } else {
            in.unread(b);

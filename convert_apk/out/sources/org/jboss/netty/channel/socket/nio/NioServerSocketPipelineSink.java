...
class NioServerSocketPipelineSink extends AbstractNioChannelSink {
    ...
    /* renamed from: id */
    final int f123id = nextId.incrementAndGet();
    ...
    private void handleServerSocket(ChannelEvent e) {
        if (e instanceof ChannelStateEvent) {
            ...
            int i = C08651.$SwitchMap$org$jboss$netty$channel$ChannelState[state.ordinal()];
            if (i != 1) {
                if (i == 2) {
                    if (value != null) {
    ...
    private void bind(NioServerSocketChannel channel, ChannelFuture future, SocketAddress localAddress) {
        ...
        try {
            ...
            DeadLockProofWorker.start(bossExecutor, new ThreadRenamingRunnable(boss, "New I/O server boss #" + this.f123id + " (" + channel + ')'));
            if (1 == 0 && 1 != 0) {
                close(channel, future);
            }
        } catch (Throwable th) {

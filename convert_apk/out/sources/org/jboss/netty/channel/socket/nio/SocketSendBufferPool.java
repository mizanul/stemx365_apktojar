...
final class SocketSendBufferPool {
    ...
    private SendBuffer acquire(FileRegion src2) {
        if (src2.getCount() == 0) {
            return EMPTY_BUFFER;
        }
        return new FileSendBuffer(src2);
    }

    private SendBuffer acquire(ChannelBuffer src2) {
        PooledSendBuffer dst;
        int size = src2.readableBytes();
        if (size == 0) {
            return EMPTY_BUFFER;
        }
        if ((src2 instanceof CompositeChannelBuffer) && ((CompositeChannelBuffer) src2).useGathering()) {
            return new GatheringSendBuffer(src2.toByteBuffers());
        }
        if (src2.isDirect()) {
            return new UnpooledSendBuffer(src2.toByteBuffer());
        }
        if (src2.readableBytes() > 65536) {
            return new UnpooledSendBuffer(src2.toByteBuffer());
        }
        ...
        if (size < remaining) {
            ...
            Preallocation preallocation = getPreallocation();
            Preallocation current3 = preallocation;
            this.current = preallocation;
        ...
        dstbuf.mark();
        src2.getBytes(src2.readerIndex(), dstbuf);
        dstbuf.reset();
        return dst;
    }
    ...
    final class PooledSendBuffer implements SendBuffer {
        ...
        PooledSendBuffer(Preallocation parent2, ByteBuffer buffer2) {
            this.parent = parent2;
            this.buffer = buffer2;
            this.initialPos = buffer2.position();
        }
        ...
        public void release() {
            ...
            if (i == 0) {
                ...
                if (parent2 != SocketSendBufferPool.this.current) {
                    ...
                    socketSendBufferPool.poolHead = new PreallocationRef(parent2, socketSendBufferPool2.poolHead);
                }
            }
        }
    }

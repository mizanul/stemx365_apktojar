...
public class LegacyLinkedTransferQueue<E> extends AbstractQueue<E> implements BlockingQueue<E>, Serializable {
    ...
    /* renamed from: MP */
    private static final boolean f155MP;
    ...
    static {
        Class<LegacyLinkedTransferQueue> cls = LegacyLinkedTransferQueue.class;
        boolean z = true;
        if (Runtime.getRuntime().availableProcessors() <= 1) {
            z = false;
        }
        f155MP = z;
        headUpdater = AtomicFieldUpdaterUtil.newRefUpdater(cls, Node.class, "head");
        tailUpdater = AtomicFieldUpdaterUtil.newRefUpdater(cls, Node.class, "tail");
        sweepVotesUpdater = AtomicFieldUpdaterUtil.newIntUpdater(cls, "sweepVotes");
    }
    ...
    private E awaitMatch(Node s, Node pred, E e, boolean timed, long nanos) {
        ...
        while (true) {
            ...
            if (w.isInterrupted() || (timed && nanos2 <= 0)) {
                if (node.casItem(obj, node)) {
                    unsplice(node2, node);
                    return obj;
                }
            }
    ...
    final class Itr implements Iterator<E> {
        ...
        private void advance(Node prev) {
            ...
            this.lastRet = prev;
            LegacyLinkedTransferQueue legacyLinkedTransferQueue = LegacyLinkedTransferQueue.this;
            Node p = prev == null ? legacyLinkedTransferQueue.head : legacyLinkedTransferQueue.succ(prev);
            while (p != null) {
                Object item = p.item;
                if (p.isData) {
                    if (!(item == null || item == p)) {

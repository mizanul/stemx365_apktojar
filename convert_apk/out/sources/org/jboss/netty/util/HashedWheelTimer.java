...
public class HashedWheelTimer implements Timer {
    ...
    /* renamed from: id */
    private static final AtomicInteger f151id = new AtomicInteger();
    ...
    public HashedWheelTimer(ThreadFactory threadFactory, long tickDuration2, TimeUnit unit, int ticksPerWheel) {
        ...
        if (threadFactory == null) {
            ...
            Set<HashedWheelTimeout>[] createWheel = createWheel(ticksPerWheel);
            ...
            if (tickDuration3 != LongCompanionObject.MAX_VALUE) {
                Set<HashedWheelTimeout>[] setArr = this.wheel;
                if (tickDuration3 < LongCompanionObject.MAX_VALUE / ((long) setArr.length)) {
                    this.roundDuration = ((long) setArr.length) * tickDuration3;
                    Worker worker2 = this.worker;
                    this.workerThread = threadFactory.newThread(new ThreadRenamingRunnable(worker2, "Hashed wheel timer #" + f151id.incrementAndGet()));
    ...
    public synchronized Set<Timeout> stop() {
        ...
        } else if (!this.shutdown.compareAndSet(false, true)) {
    ...
    public void scheduleTimeout(HashedWheelTimeout timeout, long delay) {
        ...
        long j = this.tickDuration;
        long relativeIndex = ((delay2 % this.roundDuration) / j) + ((long) (delay2 % j != 0 ? 1 : 0));
        long j2 = this.roundDuration;
        long remainingRounds = (delay2 / j2) - ((long) (delay2 % j2 == 0 ? 1 : 0));
        this.lock.readLock().lock();
        try {
            long j3 = delay2;
            int stopIndex = (int) (((long) this.mask) & (((long) this.wheelCursor) + relativeIndex));
            try {

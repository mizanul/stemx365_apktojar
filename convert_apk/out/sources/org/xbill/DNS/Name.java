...
public class Name implements Comparable, Serializable {
    ...
    private static final void copy(Name src2, Name dst) {
        if (src2.offset(0) == 0) {
            dst.name = src2.name;
            dst.offsets = src2.offsets;
            return;
        }
        int offset0 = src2.offset(0);
        int namelen = src2.name.length - offset0;
        int labels = src2.labels();
        byte[] bArr = new byte[namelen];
        dst.name = bArr;
        System.arraycopy(src2.name, offset0, bArr, 0, namelen);
        int i = 0;
        while (i < labels && i < 7) {
            dst.setoffset(i, src2.offset(i) - offset0);
            i++;
        }
        ...
        if (str.equals("")) {
            ...
            if (digits > 0 && digits < 3) {
                ...
                if (name2 != null && !absolute) {
                    appendFromString(str, name2.name, i, origin.getlabels());
                }
            } else {
                throw parseException(str, "bad escape");
            }
    ...
    public Name(Name src2, int n) {
        int slabels = src2.labels();
        if (n <= slabels) {
            this.name = src2.name;
            setlabels(slabels - n);
            int i = 0;
            while (i < 7 && i < slabels - n) {
                setoffset(i, src2.offset(i + n));
                i++;
            }
            return;
        }

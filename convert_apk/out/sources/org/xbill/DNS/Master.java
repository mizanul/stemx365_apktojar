...
public class Master {
    ...
    Master(File file2, Name origin2, long initialTTL) throws IOException {
        ...
        if (origin2 == null || origin2.isAbsolute()) {
            this.file = file2;
            this.f212st = new Tokenizer(file2);
            this.origin = origin2;
    ...
    private void parseTTLClassAndType() throws IOException {
        ...
        if (value3 < 0) {
            Tokenizer tokenizer = this.f212st;
            ...
            throw this.f212st.exception("missing TTL");
        }
    }
    ...
    private void startGenerate() throws IOException {
        long step;
        String s = this.f212st.getIdentifier();
        int n = s.indexOf("-");
        if (n >= 0) {
            ...
            String nameSpec = this.f212st.getIdentifier();
            parseTTLClassAndType();
            if (Generator.supportedType(this.currentType)) {
                String rdataSpec = this.f212st.getIdentifier();
                this.f212st.getEOL();
                this.f212st.unget();
                long j = step;
                int i = n2;
            ...
            Tokenizer tokenizer2 = this.f212st;
        ...
        Tokenizer tokenizer3 = this.f212st;
    ...
    public Record _nextRecord() throws IOException {
        ...
        while (true) {
            Tokenizer.Token token = this.f212st.get(true, false);
            if (token.type == 2) {
                Tokenizer.Token next = this.f212st.get();
                if (next.type != 1) {
                    if (next.type == 0) {
                        return null;
                    }
                    ...
                    if (record != null) {
                        ...
                        throw this.f212st.exception("no owner");
                    }
                }
                ...
                if (token.value.charAt(0) == '$') {
                    ...
                    if (s.equalsIgnoreCase("$ORIGIN")) {
                        this.origin = this.f212st.getName(Name.root);
                        this.f212st.getEOL();
                    } else if (s.equalsIgnoreCase("$TTL")) {
                        this.defaultTTL = this.f212st.getTTL();
                        this.f212st.getEOL();
                    } else if (s.equalsIgnoreCase("$INCLUDE")) {
                        String filename = this.f212st.getString();
                        ...
                        Tokenizer.Token token2 = this.f212st.get();
                        if (token2.isString()) {
        ...
        parseTTLClassAndType();
        Record fromString = Record.fromString(name, this.currentType, this.currentDClass, this.currentTTL, this.f212st, this.origin);

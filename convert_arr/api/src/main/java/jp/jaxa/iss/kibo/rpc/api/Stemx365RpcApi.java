...
public class Stemx365RpcApi {
	
	private static final Log logger = LogFactory.getLog(Stemx365RpcApi.class);
    ...
    private static Stemx365RpcApi instance = null;
   ...
   protected ExternalValueHandler exHandler = ExternalValueHandler.getInstance();
    ...
    private void sendExceptionMessage(String errmsg, Exception err) {
      try {
         ...
         logger.error("Stemx365RpcApi [sendExceptionMessage] Failed to send error message.", var4);
      }
     ...
     * to issue commands.
     */
    private Stemx365RpcApi(KiboRpcService startGuestScienceService) {
         logger.info(">>>>>>> API STARTED >>>11>>>>>>>");
         int build_number=exHandler.getBuildNumber();
         logger.info(">>>>>>> build_number >>>>>>>" + build_number);
      ...
      //   this.gsService.sendData(MessageType.JSON, "data", "some name");
         logger.info(">>>>>>> API STARTED >>> 33 >>>>>>>");
        try {
            // Get the robot
            logger.info(">>>>>>> API STARTED >> 44 >>>>>>>>");
            robot = factory.getRobot();
            // Bitmap ret = this.gsService.getBitmapNavCam();
            // logger.info(">>>>>>> API STARTED >> 555 >>>>>>>>");
            // boolean ud = this.gsService.undocked();
            // logger.info(">>>>>>> API STARTED >> 666 >>>>>>>>");
     ...
     */
    public static Stemx365RpcApi getInstance(KiboRpcService startGuestScienceService) {
        if (instance == null) {
            instance = new Stemx365RpcApi(startGuestScienceService);
        }
        return instance;
    }
    ...
    public Kinematics getTrustedRobotKinematics() {
      ...
      if (front) {
         logger.info("KiboRpcApi [Start] flashlightControl: front");
         logger.info("KiboRpcApi Parameters: brightness == " + brightness);
         flloc = FlashlightLocation.FRONT;
      } else {
         logger.info("KiboRpcApi [Start] flashlightControl: back");
         logger.info("KiboRpcApi Parameters: brightness == " + brightness);
      ...
      logger.info("KiboRpcApi [Finish] flashlightControlFront");
    ...
    public Result flashlightControlFront(float brightness) {
      ...
      try {
         logger.info("setFlashlightBrightness >>>>>11>>>>>");
         logger.info(FlashlightLocation.FRONT);
         logger.info(brightness);
         pendingResult = this.robot.setFlashlightBrightness(FlashlightLocation.FRONT, brightness);
         logger.info("setFlashlightBrightness >>>>22>>>>>>");
   ...
   public Result flashlightControlBack(float brightness) {
    ...
    public Result moveTo3(Point goalPoint, Quaternion orientation, boolean printRobotPosition) {
        ...
        if (result.hasSucceeded()) {
            ...
            logger.info("=============== MOVE TO RESULT SUCEEDED =============="); 
     ...
     public Result moveTo(final Point goalPoint, final Quaternion orientation, final boolean printRobotPosition) {
        logger.info("Stemx365RpcApi [Start] moveTo");
        logger.info("Stemx365RpcApi Parameters: goalPoint == " + goalPoint + ", orientation == " + orientation + ", printRobotPosition == " + printRobotPosition);
        setPlanner(PlannerType.TRAPEZOIDAL);
        if (goalPoint == null) {
            logger.info("Stemx365RpcApi [moveTo] goalPoint == " + goalPoint);
             logger.error("Stemx365RpcApi [moveTo] goalPoint is invalid.");
            return null;
        }
        if (orientation == null) {
            logger.info("Stemx365RpcApi [moveTo] orientation == " + orientation);
             logger.error("Stemx365RpcApi [moveTo] orientation is invalid.");
            return null;
        }
        logger.info(">>>>>>>>>>>>>>>>> PlannerType.TRAPEZOIDAL >>>>>>>>>>>>");
      //   if (!this.setPlanner(PlannerType.TRAPEZOIDAL)) {
      //        logger.error("Stemx365RpcApi [moveTo] Cannot set planner.");
      //       return null;
      //   }
        logger.info("Stemx365RpcApi [moveTo] Planner is set to " + this.plannerType.toString() + ".");
        logger.info(">>>>>>>>>>>>>>>>> STOP ALL MOTION >>>>>>>>>>>>");
        Result result = this.stopAllMotion();
        if (result == null) {
            logger.info("ERROR:: Stemx365RpcApi [moveTo] Cannot stop all motion.");
            return null;
        }
        if (result.hasSucceeded()) {
            logger.info(">>>>>>>>>>>>>>>>> STOP ALL MOTION SUCCEEDED>>>>>>>>>>>>");
            logger.info("Stemx365RpcApi [moveTo] Planner is " + this.plannerType.toString() + ".");
            logger.info("Stemx365RpcApi [moveTo] Moving the bee.");
            // PendingResult pendingResult = null;
            // try {
            ...
            //      //logger.debug("Stemx365RpcApi [moveTo] OnSimulation: " + this.gsService.getOnSimulation());
            //     if (this.isActiveTarget()) {
            //          //logger.debug("Stemx365RpcApi [moveTo] Set LaserActivationFlag: true");
        ...
        logger.info("Stemx365RpcApi [Finish] moveTo");
        return result;
    }
    

      private boolean isActiveTarget() {
         //logger.debug("Stemx365RpcApi [Start] isActiveTarget");
        boolean isActive = false;
        final Kinematics k = this.getTrustedRobotKinematics();
        if (k == null) {
            logger.info("Stemx365RpcApi [isActiveTarget] k == " + k);
             logger.error("Stemx365RpcApi [isActiveTarget] Cannot get robot kinematics.");
            return false;
        }
        final Point currPosition = k.getPosition();
        if (currPosition == null) {
            logger.info("Stemx365RpcApi [isActiveTarget] currPosition == " + currPosition);
             logger.error("Stemx365RpcApi [isActiveTarget] Cannot get current position.");
            return false;
        }
        ...
        for (final int activeTargetId : activeTargets) {
            ...
            if (distance <= 0.30000001192092896 && distance <= smallerDistance) {
                 //logger.debug("Stemx365RpcApi [isActiveTarget] Near active target [target_id: " + activeTargetId + "]");
                 //logger.debug("Stemx365RpcApi [isActiveTarget] Set nearbyTargetId [target_id: " + activeTargetId + "]");
                smallerDistance = distance;
                this.nearbyTargetId = activeTargetId;
                isActive = true;
            }
        }
         //logger.debug("Stemx365RpcApi [Finish] isActiveTarget");
        return isActive;
    }
    

   public List<Integer> getActiveTargets() {
        logger.info("Stemx365RpcApi [Start] getActiveTargets");
        final List<Integer> list = this.gsService.getActiveTargets();
        logger.info("Stemx365RpcApi [Finish] getActiveTargets");
        return list;
    }
    ...
    public boolean setPlanner(PlannerType plannerType) {
        ...
        if (result.hasSucceeded()) {
            logger.info(">>>>>>>>>>>>>>>>> SET PLANNER SUCCEEDED >>>>>>>>>>>>");
 ...
 public boolean startMission() {
      logger.info(">>>>>>>>>>>>>>>>> KRPC 4 MISSION START >>>>>>>>>>>>");
        logger.info("Stemx365RpcApi [Start] startMission");
        ...
        try {
            ...
            this.gsService.sendData(MessageType.JSON, "data", data.toString());
            logger.info("Stemx365RpcApi Undock Start: " + time);
            final PendingResult pendingResult = this.robot.undock();
            int timeout = 100;
            if (!this.gsService.getOnSimulation()) {
                //logger.debug("Stemx365RpcApi set timeout -1");
                timeout = -1;
            }
            ...
            if (result != null && result.hasSucceeded()) {
                ...
                logger.info("Stemx365RpcApi Undock Finish: " + time);
            }
            else {
                data.put("failed", (Object)"Undock failed");
                time = this.df.format(new Date(System.currentTimeMillis()));
                 logger.error("Stemx365RpcApi Undock failed: " + time);
            }
            this.gsService.sendData(MessageType.JSON, "data", data.toString());
            if (isSuccess) {
                time = this.df.format(new Date(System.currentTimeMillis()));
                ...
                this.gsService.sendData(MessageType.JSON, "data", data.toString());
                logger.info("Stemx365RpcApi Mission Start: " + time);
            }
            if (!this.calledMoveTo) {
                this.gsService.startMission();
            }
            else {
                 //logger.debug("Stemx365RpcApi [startMission] astrobee is moved. Unable to retry the start mission");
            }
        }
        ...
        logger.info("Stemx365RpcApi [Finish] startMission");
        return isSuccess;
    }
    ...
    public Result laserControl(boolean state) {
      ...
      try {
         if (state) {
            ...
            logger.info(">>>>>>>>>>>>>>>>> laserControl 13333 >>>>>>>>>>>>");
            try {
               Thread.sleep(2000L);
            } catch (InterruptedException var4) {
               logger.error("Stemx365RpcApi [getTrustedRobotkinematics] It was not possible to get a trusted kinematics. Sorry.");
               return null;
            }
         } else {
            pendingResult = this.robot.powerOffItem(PoweredComponent.LASER_POINTER);
            logger.info(">>>>>>>>>>>>>>>>> laserControl 2 >>>>>>>>>>>>");
         }
    ...
    public Bitmap getBitmapNavCam() {
      Bitmap ret = this.gsService.getBitmapNavCam();
      if (ret == null) {
         logger.error("Stemx365RpcApi [getBitmapNavCam] It was not possible to get a Bitmap from Nav Cam.");
      }
    ...
    public Mat getMatNavCam() {
      ...
      if (ret == null) {
         logger.error("Stemx365RpcApi [getMatNavCam] It was not possible to get a Mat from Nav Cam.");
      }
   ...
   public Mat getMatDockCam() {
      ...
      if (ret == null) {
         logger.error("Stemx365RpcApi [getMatDockCam] It was not possible to get a Mat from Dock Cam.");
      }
   ...
   public Kinematics getRobotKinematics() {
      logger.info("Stemx365RpcApi [getRobotKinematics] Waiting for robot to acquire position.");
   ...
   public ImuResult getImu() {
      ...
      if (ret == null) {
         logger.error("Stemx365RpcApi [getImu] It was not possible to get a Imu result data.");
      }
...
public boolean takeSnapshot(int targetNumber, boolean hasTakenSnapshot, int approach_times) {
      try {
         logger.info("KiboRpcApi [Start] takeSnapshot");
         if (hasTakenSnapshot) {
            logger.info("KiboRpcApi [takeSnapshot] Make error message");
            String err_message = "takeSnapshot can execute once.";
            JSONObject data = new JSONObject();
            data.put("error", err_message);
            this.gsService.sendData(MessageType.JSON, "data", data.toString());
            logger.info("KiboRpcApi [Finish] takeSnapshot");
            return true;
         } else {
            for(int i = 1; i <= approach_times; ++i) {
               logger.info("KiboRpcApi [takeSnapshot] Count: " + i);
               long startTime = System.currentTimeMillis();
               JSONObject data = new JSONObject();
               if (i == 1) {
                  logger.info("KiboRpcApi [takeSnapshot] Make start message");
                  JSONObject data2 = new JSONObject();
                  if (targetNumber == 1) {
                     logger.info("KiboRpcApi [takeSnapshot] Make target1 timestamp");
                     ...
                     logger.info("KiboRpcApi [takeSnapshot] Make target2 timestamp");
               ...
               if (!this.gsService.getOnSimulation()) {
                  logger.info("KiboRpcApi [takeSnapshot] getOnSimulation: False");
                  logger.info("KiboRpcApi [takeSnapshot] Save bitmap");
                  Bitmap image = this.getBitmapNavCam();
                  this.saveBitmap(image, String.valueOf(i));
               }

               if (targetNumber == 1) {
                  logger.info("KiboRpcApi [takeSnapshot] Make target1 snapshot");
                  data.put("status", "Taking a snapshot of Target1");
               } else {
                  logger.info("KiboRpcApi [takeSnapshot] Make target2 snapshot");
               ...
               for(long currentTime = System.currentTimeMillis(); currentTime - startTime <= 1000L; currentTime = System.currentTimeMillis()) {
                  if (this.gsService.getOnSimulation()) {
                     logger.info("KiboRpcApi [takeSnapshot] getOnSimulation: True");
                     logger.info("KiboRpcApi [takeSnapshot] Sleep inteval");
                     Thread.sleep(10L);
                  } else {
                     logger.info("KiboRpcApi [takeSnapshot] getOnSimulation: False");
                     logger.info("KiboRpcApi [takeSnapshot] Sleep inteval");
            ...
            JSONObject data = new JSONObject();
            if (targetNumber == 1) {
               logger.info("KiboRpcApi [takeSnapshot] Make target1 finish message");
               data.put("status", "Taking snapshots of Target1 finish");
            } else {
               logger.info("KiboRpcApi [takeSnapshot] Make target2 finish message");
   ...
   public void saveMatImage(Mat image, String imageName) {
      try {
         logger.info("KiboRpcApi [Start] saveMatImage");
         ...
         if (checkArgs) {
            logger.info("KiboRpcApi Parameters: image, imageName == " + imageName);
            if (this.gsService.getOnSimulation()) {
               ...
               if (list.length >= 50) {
                 ...
                 logger.error("KiboRpcApi [saveMatImage] The size is too large.");
                  ...
                  logger.info("KiboRpcApi [saveMatImage] Save mat image");
                ...
                logger.info("KiboRpcApi [saveMatImage] getOnSimulation: False");
            }
         }

         logger.info("KiboRpcApi [Finish] saveMatImage");
       ...
       public List<Long> getTimeRemaining() {
        logger.info("Stemx365RpcApi [Start] getTimeRemaining");
        final List<Long> list = this.gsService.getTimeRemaining();
        logger.info("Stemx365RpcApi [Finish] getTimeRemaining");
        return list;
    }
   ...
   private void saveImage(Bitmap image, String imageName, File file) throws SecurityException, IOException, NullPointerException {
    ...
    private void setSignalStateIncorrectPattern() {
         //logger.debug("Stemx365RpcApi [Start] setSignalStateIncorrectPattern");
        //this.gsService.setSignalState((byte)7, 2);
         //logger.debug("Stemx365RpcApi [Finish] setSignalStateIncorrectPattern");
    }

   public void notifyGoingToGoal() {
        logger.info("Stemx365RpcApi [Start] notifyGoingToGoal");
        try {
             //logger.debug("Stemx365RpcApi [notifyGoingToGoal] Start moving to the goal");
        ...
        logger.info("Stemx365RpcApi [Finish] notifyGoingToGoal");
    }
   ...
   public boolean reportMissionCompletion(final String report) {
    ...
    private void saveBitmap(final Bitmap saveImage, final String phaseNumber, final String targetId) throws SecurityException, IOException, NullPointerException {
         //logger.debug("Stemx365RpcApi [Start] saveBitmap");
        final String filepath = this.gsService.getGuestScienceDataBasePath() + "/immediate/JudgeImages";
        final File file = new File(filepath);
        if (!file.exists()) {
             //logger.debug("Stemx365RpcApi [saveBitmap] Make save directory");
            file.mkdir();
        }
        ...
        out.flush();
        out.close();
         //logger.debug("Stemx365RpcApi [Finish] saveBitmap");
    }
...
public void reportPoint1Arrival() {
      try {
         logger.info("KiboRpcApi [Start] reportPoint1Arrival");
         JSONObject data;
         if (this.finishing) {
            logger.info("KiboRpcApi [reportPoint1Arrival] Already finished");
            data = new JSONObject();
            data.put("status", "Already Finished");
            this.gsService.sendData(MessageType.JSON, "data", data.toString());
         } else {
            logger.info("KiboRpcApi [reportPoint1Arrival] Do getRobotKinematics");
            data = new JSONObject();
            String time = this.df.format(new Date(System.currentTimeMillis()));
            time = this.df.format(new Date(System.currentTimeMillis()));
            data.put("t_stamp_start", time);
    ...
    public void takeTargetSnapshot(final int targetId) {
        logger.info("Stemx365RpcApi [Start] takeTargetSnapshot");
        logger.info("Stemx365RpcApi Parameters: targetId == " + targetId);
        try {
            final List<Integer> activeTargets = this.gsService.getActiveTargets();
            ...
            if ((this.gsService.getOnSimulation() && activeTargets.contains(targetId) && this.nearbyTargetId == targetId) || (!this.gsService.getOnSimulation() && activeTargets.contains(targetId))) {
                int approach_time = 1;
                if (!this.gsService.getOnSimulation()) {
                     //logger.debug("Stemx365RpcApi [takeTargetSnapshot] getOnSimulation: False");
                     //logger.debug("Stemx365RpcApi [takeTargetSnapshot] Set approach_time");
                    approach_time = 2;
                }
                ...
                final JSONObject data = new JSONObject();
                 logger.error("Stemx365RpcApi [takeTargetSnapshot] Fail to deactivate the target.");
        ...
        logger.info("Stemx365RpcApi [Finish] takeTargetSnapshot");
    }
...
private boolean takeSnapshot(final int phaseNumber, final int targetId, final int approach_times) {
        try {
             //logger.debug("Stemx365RpcApi [Start] takeSnapshot");
            JSONObject data = null;
            for (int i = 1; i <= approach_times; ++i) {
                 //logger.debug("Stemx365RpcApi [takeSnapshot] Count: " + i);
                final long startTime = System.currentTimeMillis();
                if (i == 1) {
                    data = new JSONObject();
                     //logger.debug("Stemx365RpcApi [takeSnapshot] Make start message");
                    ...
                    if (!this.gsService.targetDeactivation(targetId)) {
                         logger.error("Stemx365RpcApi [takeSnapshot] Fail to deactivate the target.");
                        return false;
                    }
                }
                data = new JSONObject();
                 //logger.debug("Stemx365RpcApi [takeSnapshot] Make target snapshot");
                data.put("status", (Object)"Taking a snapshot of Target");
                data.put("target_info", (Object)this.getTargetInfoObject(phaseNumber, targetId));
                this.gsService.sendData(MessageType.JSON, "data", data.toString());
                if (!this.gsService.getOnSimulation()) {
                     //logger.debug("Stemx365RpcApi [takeSnapshot] getOnSimulation: False");
                     //logger.debug("Stemx365RpcApi [takeSnapshot] Save bitmap");
                    final Bitmap image = this.getBitmapNavCam();
                    this.saveBitmap(image, String.valueOf(phaseNumber), String.valueOf(targetId));
                }
                for (long currentTime = System.currentTimeMillis(); currentTime - startTime <= 1000L; currentTime = System.currentTimeMillis()) {
                    if (this.gsService.getOnSimulation()) {
                         //logger.debug("Stemx365RpcApi [takeSnapshot] getOnSimulation: True");
                         //logger.debug("Stemx365RpcApi [takeSnapshot] Sleep inteval");
                        Thread.sleep(10L);
                    }
                    else {
                         //logger.debug("Stemx365RpcApi [takeSnapshot] getOnSimulation: False");
            ...
            data = new JSONObject();
             //logger.debug("Stemx365RpcApi [takeSnapshot] Make target finish message");
            data.put("status", (Object)"Taking snapshots of Target finish");
            data.put("target_info", (Object)this.getTargetInfoObject(phaseNumber, targetId));
            this.gsService.sendData(MessageType.JSON, "data", data.toString());
             //logger.debug("Stemx365RpcApi [Finish] takeSnapshot");
            return true;
        }
        catch (JSONException e) {
...
public void takeTarget1Snapshot() {
      logger.info("KiboRpcApi [Start] takeTarget1Snapshot");
      int approach_time = 1;
      if (!this.gsService.getOnSimulation()) {
         logger.info("KiboRpcApi [takeTarget1Snapshot] getOnSimulation: False");
         logger.info("KiboRpcApi [takeTarget1Snapshot] Set approach_time");
         approach_time = 2;
      }

      boolean result = this.takeSnapshot(1, this.hasTargetSnapshot1, approach_time);
      this.hasTargetSnapshot1 |= result;
      logger.info("KiboRpcApi [Finish] takeTarget1Snapshot");
   }
...
public void takeTarget2Snapshot() {
  ...
  public void saveBitmapImage(final Bitmap image, final String imageName) {
        try {
            logger.info("Stemx365RpcApi [Start] saveBitmapImage");
            boolean checkArgs = true;
            if (image == null) {
                 logger.error("Stemx365RpcApi [saveBitmapImage] image is null");
                checkArgs = false;
            }
            if (imageName == null) {
                 logger.error("Stemx365RpcApi [saveBitmapImage] imageName is null");
                checkArgs = false;
            }
            if (checkArgs) {
                logger.info("Stemx365RpcApi Parameters: image, imageName == " + imageName);
                if (this.gsService.getOnSimulation()) {
                     //logger.debug("Stemx365RpcApi [saveBitmapImage] getOnSimulation: True");
                     //logger.debug("Stemx365RpcApi [saveBitmapImage] Check directry");
                    final String filepath = this.gsService.getGuestScienceDataBasePath() + "/immediate/DebugImages";
                    final File file = new File(filepath);
                    if (!file.exists()) {
                         //logger.debug("Stemx365RpcApi [saveBitmapImage] Make save directry");
                        file.mkdir();
                    }
                    ...
                    if (list.length >= 50) {
                         logger.error("Stemx365RpcApi [saveBitmapImage] Can't save more than 50 images");
                    }
                    else if (size > 1228800) {
                         logger.error("Stemx365RpcApi [saveBitmapImage] The size is too large.");
                    }
                    else {
                         //logger.debug("Stemx365RpcApi [saveBitmapImage] Save bitmap image");
                        this.saveImage(image, imageName, file);
                    }
                }
                else {
                     //logger.debug("Stemx365RpcApi [saveBitmapImage] getOnSimulation: False");
                }
            }
            logger.info("Stemx365RpcApi [Finish] saveBitmapImage");
        }
        catch (Exception e) {
   ...
   public Bitmap getBitmapDockCam() {
      ...
      if (ret == null) {
         logger.info("KiboRpcApi [getBitmapDockCam] ret is null.");
      }

      logger.info("KiboRpcApi [Finish] getBitmapDockCam");
      return ret;
   }

   public void notifyRecognitionItem() {
      ////logger.debug("KiboRpcApi", "[Start] notifyRecognitionItem");

      try {
        ////logger.debug("KiboRpcApi", "[notifyRecognitionItem] Target Item recognition is complete");
         JSONObject data = new JSONObject();
      ...
      //logger.debug("KiboRpcApi", "[Finish] notifyRecognitionItem");
   }

   public boolean reportRoundingCompletion() {
      //logger.debug("KiboRpcApi", "[Start] reportRoundingCompletion");

      try {
         if (!this.reportCompletion) {
            //logger.debug("KiboRpcApi", "[reportRoundingCompletion] Make rounding is complete message");
            JSONObject data = new JSONObject();
            data.put("t_stamp_rounding_completion", this.df.format(new Date(System.currentTimeMillis())));
            ...
            //logger.debug("KiboRpcApi", "[reportRoundingCompletion] Do getRobotKinematics");
            Kinematics kinematics = this.getRobotKinematics();
            if (kinematics == null || kinematics.getPosition() == null || kinematics.getOrientation() == null) {
               //logger.error("KiboRpcApi", "[reportRoundingCompletion] It was not possible to get a kinematics.");
               if (kinematics == null) {
                  logger.error("KiboRpcApi reportRoundingCompletion] kinematics is null.");
               }

               if (kinematics.getPosition() == null) {
                  logger.error("KiboRpcApi [reportRoundingCompletion] kinematics.getPosition() is null.");
               }

               if (kinematics.getOrientation() == null) {
                  logger.error("KiboRpcApi [reportRoundingCompletion] kinematics.getOrientation() is null.");
               }
            ...
            for(int i = 0; i < 2; ++i) {
               //logger.debug("KiboRpcApi", "[reportRoundingCompletion] Count: " + i);
      ...
      //logger.debug("KiboRpcApi", "[Finish] reportRoundingCompletion");
      return true;
   }
